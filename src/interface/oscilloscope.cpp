...
int Oscilloscope::_interpolateAndCenter(int16_t &y_o, const int16_t y_n,
					float intp)
{
...
void Oscilloscope::init(glm::ivec2 pos, glm::ivec2 shape, size_t samples)
{
	_pos = pos;
	_shape = shape;
...
void Oscilloscope::draw(const int16_t *buffer, Renderer *ren, float intp)
{
	// Draw background and bounding box
	SDL_SetRenderDrawColor(ren->getRenderer(), _bgCol.r, _bgCol.g, _bgCol.b,
			       _bgCol.a);
	...
	int16_t prevY = _interpolateAndCenter(oldDisplay[0], buffer[0], intp);

	for (int i = 1; i < _shape.x; ++i) {
		size_t scaled_idx = static_cast<size_t>(i * _scale);

		int16_t thisY = _interpolateAndCenter(oldDisplay[i],
						      buffer[scaled_idx], intp);

		SDL_RenderDrawLine(ren->getRenderer(), _pos.x + i - 1, prevY,
				   _pos.x + i, thisY);
